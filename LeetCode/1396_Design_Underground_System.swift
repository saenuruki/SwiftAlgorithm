...
class UndergroundSystem {
    ...
    func getAverageTime(_ startStation: String, _ endStation: String) -> Double {
        let customers = transactions.filter({ $0.startStation == startStation && $0.endStation == endStation && $0.travelTime > 0 })
        guard customers.count > 0 else { return 0.0 }
        return customers.map({ $0.travelTime }).reduce(0.0, +) / Double(customers.count)
    }
}
