class Solution {
    func scheduleCourse(_ courses: [[Int]]) -> Int {        
        // 先ず last dayが早い順にsortする
        let sortedCourses = courses.sorted { $0[1] < $1[1] }
        // Data Structure は course修了日数を比較するHeap
        var maxHeap = Heap<Int>(sort: > )
        var day = 0

        for course in sortedCourses {
            let duration = course[0]
            let maxDays = course[1]

            // 現在取得Course数に今回の日数を追加してもLastDayに達しない場合は追加する
            if day + duration <= maxDays {
                maxHeap.insert(duration)
                day += duration
            // 最も大きい日数のCourseを取り除いて、今回の日数を追加しても、全体の数が変化しない＆他にも追加できる可能性がある
